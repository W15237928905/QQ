...
void AiChat::on_sending_Btn_clicked()
{
    ...
    item->setSizeHint(QSize(250,send_item->return_SendHeight()+50));
...
int AiChat::get_Aianswer(const QString &response)
{
    //初始化气泡并输出
    QListWidgetItem *item=new QListWidgetItem(ui->listWidget);
    Recvbox *send_item=new Recvbox(this);
    send_item->set_RecvText(response);
    item->setSizeHint(QSize(250,send_item->return_RecvHeight()+50));
    ui->listWidget->setItemWidget(item,send_item);
