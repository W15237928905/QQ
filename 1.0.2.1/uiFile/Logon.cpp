...
bool Logon::testing_Input(const QString &number,const QString &password)
{
    ...
    if(QValidator::Acceptable != usernumber_Validator->validate(input_Usernumber,pos) ||
            input_Usernumber == nullptr || !my_SQL.testing_SqlQuery("user", "number",number)){
        //验证号码是否存在且正确
        ...
        return false;
            ...
            input_Password == nullptr || !my_SQL.testing_SqlQuery("user", "password",password)){
        //验证密码是否正确
