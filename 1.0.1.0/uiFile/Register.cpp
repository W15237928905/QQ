...
void Register::on_register_Btn_clicked()
{
    ...
    if(!getEdit_InputSecure()){
    ...
    else if(mySql.get_SqlQuery("user", "number",input_Number)){
        //检测账号是否存在
        ...
        QString query = QStringLiteral("INSERT INTO user (id, name, number, password, sex, headimg)"
                                       "VALUES ('%1', '%2', '%3', '%4', '%5', '%6')")
                            .arg(mySql.get_RowCount("user")+1).arg(input_Name).arg(input_Number)
                            .arg(input_Password).arg(input_Sex).arg("");
        if(mySql.add_SqkQuery(query)){
...
void Register::on_headimg_Btn_clicked()
{
    ...
    /*//初始化姓名，个性签名，头像
    QPixmap originalPixmap(":/new/user/user/My.jpg");
    originalPixmap = originalPixmap.scaled(ui->headimg_Btn->width(),
                                           ui->headimg_Btn->height(), Qt::KeepAspectRatio);
    ui->headimg_Btn->setIcon(originalPixmap);*/
